...
,jezici.dugme11,jezici.dugme13,jezici.dugme14,jezici.dugme15,jezici.dugme16,jezici.dugme17,jezici.dugme18 FROM jezici WHERE jezici.id=$ajdijezika" ;

 $izjava = $db->prepare($upit) ;
 $izjava->execute();
 $izjava->store_result();
$izjava->bind_result($ajdjezika,$dugme1,$dugme2,$dugme3,$dugme4,$dugme5,$dugme6,$dugme7,$dugme8,$dugme9,$dugme10,$dugme11,$dugme13,$dugme14,$dugme15,$dugme16,$dugme17,$dugme18);
while($izjava->fetch()){ }
...
<div class="container-fluid" >
  ...
  <form action="tcpdf/examples/svedozvole1.php" method="post">
     ...
     ';
      ...
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0">
          ...
          <form action="public.php" method="POST">
          <input type="hidden" value="'.$ajdijezika.'" name="flexRadioDefault"/>
          <button type="submit" class="btn btn-sm btn-outline-secondary" id="unosno" name="unosno">'.' '.$dugme1.'</button>
          </form>
          <form action="starigrad.php" method="POST">
...
include('db.php');
 ...
 $upit = "SELECT * FROM mz WHERE ured='$id' " ;
...
// $izjava->bind_result($ID,$naziv,$model,$dobavljacID,$proizvodjacID,$serviserID,$ugovorID,$odjelID,$zgradaID,$katID,$soba,$invbroj,$serijskibroj,$nabavnasifra,$datumzaduzenja,$istekgarancije,$preventivnostart,$preventivnointerval,$napomena,$status);
 ...
 echo '<div class="thumbnail col-lg-12 col-md-12 col-xs-12 col-sm-12" style="margin-top:35px;"><div class="caption"><table class="table table-striped">
 <thead><th></th><th>'.$dugme18.'</th><th>'.$dugme17.'</th></thead>';
 while($izjava->fetch()){

    echo '<tr>
    <td><form action="dozvoleprikaz.php" method="POST"><input type="hidden" value="'.$id.'" name="idd"><input type="hidden" value="2021" name="slika"><input type="hidden" value="'.$ajdijezika.'" name="flexRadioDefault"/><input type="submit" value="'.$dugme5.'" class="btn btn-sm btn-warning" data-toggle="modal" data-target="#myModal2"></form></td>
     <td>'.$naziv.'</td>
     <td>'.$telefon.'</td>
     </tr>';
